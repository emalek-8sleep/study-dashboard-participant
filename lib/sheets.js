...
export async function fetchSheet(tabName, sheetId) {
  ...
  // Add a per-minute cache-buster so Google's CDN doesn't serve stale data
  // (especially important for recently-written values like PINs).
  const cacheBuster = Math.floor(Date.now() / 60000);
  const url = `https://docs.google.com/spreadsheets/d/${id}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(tabName)}&_t=${cacheBuster}`;
...
export async function getStudyConfig(sheetId) {
  ...
  const url = `https://docs.google.com/spreadsheets/d/${id}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent('Study Config')}`;
  try {
    const res = await fetchWithTimeout(url);
