...
export async function fetchSheet(tabName, sheetId) {
  ...
  // Use export?format=csv for a fresh read every time.
  // gviz/tq is aggressively cached by Google's CDN regardless of Cache-Control
  // headers; export?format=csv is served directly and honours cache-busting.
  // A unique _t param per request ensures no stale data (critical for PIN reads).
  const cacheBuster = Date.now();
  const url = `https://docs.google.com/spreadsheets/d/${id}/export?format=csv&sheet=${encodeURIComponent(tabName)}&_t=${cacheBuster}`;
...
export async function getStudyConfig(sheetId) {
  ...
  const url = `https://docs.google.com/spreadsheets/d/${id}/export?format=csv&sheet=${encodeURIComponent('Study Config')}&_t=${Date.now()}`;
  try {
    const res = await fetchWithTimeout(url);
